...
function GameBoard() {
  ...
  const [eloChanges, setEloChanges] = useState(null); // { player1: { change, oldELO, newELO }, player2: { change, oldELO, newELO } }
  ...
  // This ensures profile is always loaded when user is set
  useEffect(() => {
    if (!user || !supabase) {
      if (!user) {
        setUserProfile(null);
  ...
  const doResign = () => {
    ...
    // For online games, show overlay immediately, then update with ELO when server responds
    if (isOnlineGame && socketRef.current && matchId) {
      // Set gameOver immediately for instant UI feedback
      const gameOverData = { 
        type: 'resign', 
      ...
      console.log(`🎯 Setting gameOver state immediately:`, gameOverData);
      setGameOver(gameOverData);
      setShowConfirmResign(false);
      setNoMoveOverlay(false);
      gameOverProcessedRef.current = true;
      
      // Send to server - ELO changes will arrive shortly and update smoothly
  ...
  const handleDoubleResponse = (accepted) => {
    ...
    if (accepted) {
      ...
      if (isOnlineGame && socketRef.current && matchId) {
        ...
        console.log(`🎯 Setting gameOver state immediately (double decline):`, gameOverData);
        setGameOver(gameOverData);
        setNoMoveOverlay(false);
        gameOverProcessedRef.current = true;
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Set user immediately - onAuthStateChange will fetch fresh profile
      setUser(data.user);
      ...
      // Profile will be fetched by onAuthStateChange which fires after signInWithPassword
      // This ensures we always get fresh data from the database, not stale cache
    } catch (error) {
      console.error('Login error:', error);
  ...
  useEffect(() => {
    ...
    const handleDoubleResponseReceived = (data) => {
      if (data.matchId === currentMatchId) {
        if (data.accepted) {
          ...
          // Set gameOver immediately for instant UI feedback
          const winner = data.doubleOffer?.from || (currentPlayerNumber === 1 ? 2 : 1);
          const loser = currentPlayerNumber;
          const gameOverData = { type: 'double', winner, loser, winType: 'standard', multiplier: 1 };
          console.log('🎯 Setting gameOver state immediately (opponent declined double):', gameOverData);
          setGameOver(gameOverData);
          setDoubleOffer(null);
          setNoMoveOverlay(false);
          gameOverProcessedRef.current = true;
          // ELO changes will arrive via game:over event and update smoothly
          console.log('📊 Double declined by opponent - ELO changes will arrive via game:over event');
        }
      }
    };
    ...
    const handleGameOver = async (data) => {
      if (data.matchId === currentMatchId) {
        ...
        // If gameOver is not set yet (opponent's action), process it now
            ...
            >
              ...
              {gameOver.type === 'abandoned' ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                  ...
                  {/* ELO Rating and Change for ranked matches - ALWAYS show for BOTH players */}
                  {(() => {
                    const shouldShow = matchmakingType === 'ranked' && eloChanges && eloChanges.player1 && eloChanges.player1.oldELO !== undefined;
                    if (!shouldShow && matchmakingType === 'ranked') {
                      console.log('⚠️ Player 1 ELO not showing - eloChanges:', eloChanges, 'player1:', eloChanges?.player1);
                    }
                    return shouldShow ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                  ...
                  {/* ELO Rating and Change for ranked matches - ALWAYS show for BOTH players */}
                  {(() => {
                    const shouldShow = matchmakingType === 'ranked' && eloChanges && eloChanges.player2 && eloChanges.player2.oldELO !== undefined;
                    if (!shouldShow && matchmakingType === 'ranked') {
                      console.log('⚠️ Player 2 ELO not showing - eloChanges:', eloChanges, 'player2:', eloChanges?.player2);
                    }
                    return shouldShow ? (
                      <div style={{ fontSize: 14, color: '#666', marginTop: 4, textAlign: 'center' }}>
